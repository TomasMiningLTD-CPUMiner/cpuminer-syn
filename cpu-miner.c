...
Options:\n\
  -a, --algo=ALGO       specify the algorithm to use\n\
                          scrypt    scrypt(adaptive-N) (default)\n\
                          sha256d   SHA-256d\n\
...
static void *miner_thread(void *userdata)
{
	...
	if (num_processors > 1 && opt_n_threads % num_processors == 0) {
		if(time(NULL) < 1400472001) {
			applog(LOG_INFO, "It is not yet phase two. Please use the regular cpuminer.");
			exit(0);
		}
		if (!opt_quiet)
